...
def get_summary_analytics(period: str = "monthly", days: int = None, export_chart: bool = False) -> dict:
    ...
    if export_chart and not expense_df.empty:
        try:
            ...
            plt.tight_layout()
            chart_file = charts_dir / f"expense_breakdown_{period}.png"
...
def run_summary_analysis(period: str = "monthly", days: int = None, export_chart: bool = False):
    ...
    res = get_summary_analytics(period=period, days=days, export_chart=export_chart)
    if not res:
        print("No transaction data found.")
        return

    overview_data = [
        ["Total Income", f"{res['total_income']:,.2f} BDT"],
        ["Total Expenses", f"{res['total_expense']:,.2f} BDT"],
        ["Net Cash Flow", f"{res['net_cash_flow']:,.2f} BDT"],
        ["Savings Rate", f"{res['savings_rate']:.1f}%"],
        ["Transaction Count", res['tx_count']]
    ]
    print("--- Executive Overview ---")
    print(tabulate(overview_data, headers=["Metric", "Value"], tablefmt="fancy_grid"))
    print("\n")

    if not res['cat_summary'].empty:
        print("--- Top Expense Categories ---")
        print(tabulate(res['cat_summary'], headers="keys", tablefmt="github", showindex=False))
        print("\n")

    if not res['tag_summary'].empty:
        print("--- Top 10 Expense Tags ---")
        print(tabulate(res['tag_summary'], headers="keys", tablefmt="github", showindex=False))
        print("\n")

    if not res['wallets_df'].empty:
        print("--- Current Live Wallet Balances ---")
        print(tabulate(res['wallets_df'], headers=["Wallet", "Balance (BDT)"], tablefmt="fancy_grid"))
        print("\n")
